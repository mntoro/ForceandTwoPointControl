...
Adafruit_MotorShield AFMSone(0x60);                                                           // Bottom-most motor shield
Adafruit_MotorShield AFMStwo(0x61);                                                           // Second from the bottom motor shield
// Adafruit_MotorShield AFMSthree(0x62);                                                      // Third from the bottom motor shield
// Adafruit_MotorShield AFMSfour(0x63);                                                       // Second from the top motor shield
// Adafruit_MotorShield AFMSfive(0x64);                                                       // Top motor shield

// Calling and naming individual motors
//Adafruit_StepperMotor *myStepper5 = AFMSfive.getStepper(125, 2);                            // Rotary stepper on motor shield 5
//Adafruit_StepperMotor *myCanstack5 = AFMSfive.getStepper(24, 1);                           // Linear stepper on motor shield 5

//Adafruit_StepperMotor *myStepper4 = AFMSfour.getStepper(125, 2);                            // Rotary stepper on motor shield 4
//Adafruit_StepperMotor *myCanstack4 = AFMSfour.getStepper(24, 1);                           // Linear stepper on motor shield 4

//Adafruit_StepperMotor *myStepper3 = AFMSthree.getStepper(125, 2);                           // Rotary stepper on motor shield 3
//Adafruit_StepperMotor *myCanstack3 = AFMSthree.getStepper(24, 1);                          // Linear stepper on motor shield 3

Adafruit_StepperMotor *myStepper2 = AFMStwo.getStepper(125, 2);                               // Rotary stepper on motor shield 2
Adafruit_StepperMotor *myCanstack2 = AFMStwo.getStepper(24, 1);                              // Linear stepper on motor shield 2

Adafruit_StepperMotor *myStepper1 = AFMSone.getStepper(125, 2);                               // Rotary stepper on motor shield 1
Adafruit_StepperMotor *myCanstack1 = AFMSone.getStepper(24, 1);                              // Linear stepper on motor shield 1
...
int force() {
  ...
  Serial.println("Which finger do you want to test? (t, f, m, r, p)");                                        //1 is thumb, 2 is forefinger, 3 is middle finger, 4 is ring finger, 5 is pinky
  while (!Serial.available()){}                                                                   //Do Absolutely Nothing until something is received over the serial port
  char fingernumber = Serial.read();                                                               //fingernumber is the number associated with the finger they want to test
...
void forceone(){
  ...
  while (grams < 100) {                                                                            //Canstack motor moves until pushed down
    ...
    myCanstack1->step(100, BACKWARD, SINGLE);                                                       //continually rotate linear stepper motor
  }                                                                                                

  Serial.println("Test will end.");                                                                                          //signal a return to GUI to stop test and output number
}
...
void forcetwo(){
  ...
  while (grams < 100) {                                                                           //Canstack motor moves until pushed down
    ...
    myCanstack2->step(100, BACKWARD, SINGLE); 
  }  
...
void twopointone(){
  myStepper1->step(25, FORWARD, SINGLE);                                                          //25 is the number of steps per turn
  myCanstack1->step(100, BACKWARD, DOUBLE);                                                        //100 is temporary, should be steps needed to feel force in force test
...
void twopointtwo(){
  myStepper2->step(25, FORWARD, SINGLE);
